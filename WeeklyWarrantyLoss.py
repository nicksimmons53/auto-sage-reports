...
def runQuery(date, directory, dsn):
    ...
    sql = """
        select
        	...
        	from [MC Surfaces, Inc].[dbo].pchord
        		...
        		where
        			[MC Surfaces, Inc].[dbo].pchord.ordtyp = 3
    ...
    fileName = "LOSS REPORT " + today + ".xlsx"
    ...
    worksheet.set_header('&C&24Weekly Warranty Report Loss')
    ...
    for e1, ob, e2, fr, dsc, od, vn, on, j, jnm, t, typ, nte in data:
        ...
        worksheet.write(row, col + 6, str(j) + " - " + str(jnm), top)
